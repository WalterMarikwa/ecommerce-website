...
('Party Mix', 'Dj', 80, True),
('Hercules mix Dj', 'Dj', 60, True),
('Black Headphones', 'Headphones', 76, True),
('White Beats', 'Headphones', 100, True),
('Rave Beats', 'Headphones', 120, True),
('Key Board', 'Other', 40, True),
('Drums2.0', 'Other', 90, True),
('Violin', 'Other', 45, True);
...
UPDATE `products` PRO
SET PRO.`description_type` = 'A semi-acoustic guitar is a type of electric guitar that originates from the 1930s.'
WHERE PRO.`product_id` = 2;

UPDATE `products` PRO
SET PRO.`description_type` = 'An electric guitar converts the vibration of its strings into electrical signals.'
...
UPDATE `products` PRO
SET PRO.`description_type` = 'A wooden string instrument in the violin family.'
